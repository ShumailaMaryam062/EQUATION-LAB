---
title: 'Diff equation Solver: A Python package for solving differential equations with neural networks'
tags:
  - Python
...
``Diff equation Solver`` is a Python package built with ``PyTorch`` [@paszke2017automatic] that uses ANNs to solve ordinary and partial differential
equations (ODEs and PDEs).  During the release of ``Diff equation Solver`` we discovered that two other groups had almost simultaneously
...
``Diff equation Solver`` is designed to encourage the user to focus more on the problem domain (What is the differential equation we
need to solve? What are the initial/boundary conditions?) and at the same time allow them to dig into solution domain (What
ANN architecture and loss function should be used? What are the training hyperparameters?) when they want to.  ``Diff equation Solver`` 
...
spatial dimensions.  We are actively working on extending ``Diff equation Solver`` to support three spatial dimensions.  ``Diff equation Solver`` 
can also solve arbitrary systems of nonlinear ordinary differential equations.
Currently, ``Diff equation Solver`` is being used in a variety of research projects including to study the convergence properties of ANNs 
...
as the loss function.  In general, the $L^{2}$ loss of the residual is used.  This is the convention that ``Diff equation Solver`` follows, 
...
conditions. ``PyDEns`` uses a variation of the second approach to impose initial/boundary conditions. ``Diff equation Solver``, the
